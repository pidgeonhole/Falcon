...
COPY_UNIT_FILES=("iptables-restore" "swap" "postgres" "redis" "CodeQuiz" "nginx")
...
function git_init () {
  ...
  scp "git/post-receive/CodeQuiz" "${SSH_USER}@${SERVER_IP}:/tmp/CodeQuiz"
  scp "git/post-receive/nginx" "${SSH_USER}@${SERVER_IP}:/tmp/nginx"
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
sudo apt-get update && sudo apt-get install -y -q git
sudo rm -rf /var/git/CodeQuiz.git /var/git/CodeQuiz /var/git/nginx.git /var/git/nginx
sudo mkdir -p /var/git/CodeQuiz.git /var/git/CodeQuiz /var/git/nginx.git /var/git/nginx
sudo git --git-dir=/var/git/CodeQuiz.git --bare init
sudo git --git-dir=/var/git/nginx.git --bare init

sudo mv /tmp/CodeQuiz /var/git/CodeQuiz.git/hooks/post-receive
sudo mv /tmp/nginx /var/git/nginx.git/hooks/post-receive
sudo chmod +x /var/git/CodeQuiz.git/hooks/post-receive /var/git/nginx.git/hooks/post-receive
sudo chown ${KEY_USER}:${KEY_USER} -R /var/git/CodeQuiz.git /var/git/CodeQuiz.git /var/git/CodeQuiz /var/git/nginx.git /var/git/nginx
  '"
  echo "done!"
}
